...
output "s3-vpce-over-vpn-test-command" {
  value = "aws s3 ls s3://${local.bucket_name} --endpoint-url https://bucket.${module.shared-logging-vpc-endpoints.endpoints.s3.dns_entry[0].dns_name} --region eu-central-1"
}
